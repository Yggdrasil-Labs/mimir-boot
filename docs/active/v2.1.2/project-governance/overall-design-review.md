...
项目的 `Parent -> BOM -> Common -> Starters` 分层方向合理。v2.1.2 已完成本清单的
5 项 P1 和 5 项 P2 治理验证；版本级同源预检、报告、文档与提交归属由 T12 复核。
...
- **建议方向**：统一版本线，并用同源 POM 解析和完整 Reactor 验证证明仓库内构建一致。
- **关闭条件**：effective/flattened POM 中依赖平台与构建插件解析到同一版本，完整 Reactor
  `verify -Pci` 通过；不维护额外消费者契约工程。
- **状态**：已验证
- **验证证据**：T1 AC2；T12 AC2。
...
- **问题**：HTTP/Jackson 响应数据被 Java Serialization 泛型上界约束；异常响应工厂对
  非 Serializable 附加数据沿用当前丢弃语义。
- **建议方向**：v2.x 明确保留现有公共边界与行为，放宽上界及异常附加数据语义进入 3.0 评估。
- **关闭条件**：兼容性决策已记录；Serializable 正例编译与 JSON 回归通过，非 Serializable 用法继续
  编译失败；3.0 候选项可追踪。本版本不宣称已修复既有数据丢弃语义。
- **决策证据**：v2.1.2 Spec、Design 与 T1 AC3；3.0 候选保留在本条。
- **状态**：已关闭
...
- **领域**：bom / maintenance
- **证据位置**：`mimir-boot-bom/pom.xml`、`mimir-boot-bom/README.md` 中的支持等级分类
- **问题**：55 个显式管理坐标中有 40 个当前未被 Starter 直接消费，增加依赖升级噪音并模糊平台边界。
- **建议方向**：在“精简为已消费依赖”和“拆分独立生态 BOM”之间形成产品与兼容性决策。
- **关闭条件**：完成依赖使用清单和消费方影响评估；选定方案进入版本计划，未选方案记录原因。
- **状态**：已验证
- **验证证据**：T4 AC2；T12 AC1、AC2。宽 BOM 保留，支持等级分类已完成。

...
- **问题**：方法校验、缺少请求头、媒体类型协商、上传超限和静态资源不存在等常见客户端异常
  没有专门映射；同时必须避免把返回值校验或路径变量映射错误降级为客户端 400。
- **确认方向**：补齐 400、404、406、413、415 等 Spring 6 客户端语义；返回值方法校验和
  `MissingPathVariableException` 保持 500；继续使用现有响应工厂。

...
2026-09-20 将仅服务 v2.2.1 T9 的同步授权记录从设计库迁入[所属需求](../../v2.2.1/technical-debt-remediation/rfc-doc-sync.md)，由需求索引引用，不保留旧路径入口，也不提前归档 v2.2.1。设计库继续保留跨版本架构设计与决策。
...
- 不在文档迁移中关闭 TD-043/TD-044、不自动归档 v2.2.1、不将 RFC 改为 verified；相应动作须有各自证据。

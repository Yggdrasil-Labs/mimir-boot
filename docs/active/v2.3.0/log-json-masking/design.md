...
配置快照继续在发布时一次性构建并以原子引用生效。构建时先将已启用预置规则划分为字段型与纯值型：字段型规则的别名汇入按长度降序排列的字段名集合，纯值型规则才编译为预置正则。自定义和编程式正则仍在随后追加。转换时先扫描字段值，再对结果执行纯值和自定义正则，从而避免敏感字段值进入后续处理。
...
字段型规则的日志文本输出存在一项有意修正：值后紧接 `,`、`}`、`]` 或空白时，新的扫描器保留这些非敏感分隔符和尾部文本；旧正则可能将它们包含在匹配中并丢弃。该修正不要求接入方调整配置，测试必须固定断言 `account=alice, tail=sentinel` 输出为 `account=****, tail=sentinel`。字段型规则先于自定义/编程式正则执行，这是把既有 password/token/secret 的安全顺序扩展到全部字段型规则；后续正则只会看到 replacement，不得依赖已启用字段的原始敏感值。纯值预置规则及自定义/编程式正则仍在字段值扫描后执行。
...
| 内置或自定义正则编译失败 | 沿用当前告警并忽略无效规则；配置发布仍会以剩余有效规则构建新快照并替换旧快照，不承诺保留旧规则集合。 |

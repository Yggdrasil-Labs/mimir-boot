...
| Starter | 核心价值 | 典型场景 |
|---|---|---|
| `mimir-boot-starter-log` | 统一日志、脱敏、访问日志、MDC 链路标识输出 | 绝大多数服务 |
...
| `mimir-boot-starter-mybatis-processor` | 编译期生成 Mapper、Service、ServiceImpl | 可选；需要减少样板代码的持久层项目 |
...
- 最小 Web 服务组合
- `starter-mybatis`（数据访问基础能力）
- `starter-mybatis-processor`（可选；仅在需要编译期生成 Mapper、Service、ServiceImpl 时引入）

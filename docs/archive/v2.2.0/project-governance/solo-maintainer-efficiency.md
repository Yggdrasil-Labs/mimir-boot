...
减少重复构建、重复刷新和零散依赖 PR；本版本优先落地低维护的 Bash/Maven 门禁，文档事实自动化
按延期项继续跟踪，不以降低更新及时性、自动合并或重构发布补偿机制换取表面效率。
...
- **关闭条件**：故意修改一个版本号、Starter 数量或内部链接时 CI 失败并指出不一致项；
  `2.1.2-SNAPSHOT`、目录 `v2.2.0` 和索引 `v2.2.0` 归一后相等，恢复后通过。
- **状态**：延期
- **Owner**：Yggdrasil Labs
- **延期理由**：2026-08-12 已确认以低维护成本收缩 T2，不引入项目级 Node、YAML 解析器或长期文档
  事实脚本；当前文档事实已人工校正，但尚不能机械证明故意漂移会在 CI 中失败并定位。
- **目标版本**：v2.1.x 后续独立文档治理需求
- **当前证据**：T12 AC1、AC2 只证明当前文档和本地门禁通过，不作为 GOV-012 自动化关闭证据。

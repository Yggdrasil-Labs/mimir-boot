...
- 包装 Feign 最终选定的 `Client`，在调用前/后/异常/清理阶段调度 RPC Hook
...
适配层的自动配置同时要求 `mimir.boot.rpc.core.enabled=true`（默认）且存在 `RpcHookChain`、`RpcTracerBridge` Bean；任一条件不满足时不会创建 `RpcFeignCapability`，Feign Client 不会被本模块的 `RpcFeignClient` 包装。

## 使用示例

- 自定义 Hook：与 RPC Core 示例一致
- 自定义 Tracer：覆盖 `RpcTracerBridge`，在 HTTP 头中注入追踪上下文；Feign 只负责出站注入，HTTP 入站由 Web Starter 处理

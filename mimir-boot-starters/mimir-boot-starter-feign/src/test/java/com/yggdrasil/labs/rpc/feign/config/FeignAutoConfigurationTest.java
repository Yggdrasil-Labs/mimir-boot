...
class FeignAutoConfigurationTest {
            ...
            new ApplicationContextRunner()
                    .withConfiguration(
                            AutoConfigurations.of(
                                    RpcCoreAutoConfiguration.class, FeignAutoConfiguration.class));
    ...
    void shouldCreateRpcFeignCapability() {
        RpcFeignCapability capability =
                configuration.rpcFeignCapability(hookChain, tracerBridge, properties);
    ...
    void shouldWrapDelegateClient() {
        ...
        Client client =
                configuration
                        .rpcFeignCapability(hookChain, tracerBridge, properties)
                        .enrich(delegate);
    ...
    void skipsExplicitAdapterWhenCoreDisabled() {
        runner.withPropertyValues(
                        "mimir.boot.rpc.core.enabled=false", "mimir.boot.feign.enabled=true")
                .run(
                        context -> {
                            assertThat(context).hasNotFailed();
                            assertThat(context).doesNotHaveBean(RpcFeignCapability.class);
    ...
    void runsAfterRpcCoreAutoConfiguration() {
        AutoConfiguration autoConfiguration =
                FeignAutoConfiguration.class.getAnnotation(AutoConfiguration.class);

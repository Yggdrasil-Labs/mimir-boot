...
class FeignAutoConfigurationEndToEndTest {
    ...
    void shouldDecorateCustomClientWithoutBypassingIt() {
        runner.run(
                context -> {
                    ...
                    assertEquals(
                            1, context.getBean(RecordingRpcHook.class).beforeInvocations.get());
                });
    }
    ...
    void defaultBridgePropagatesMdcThroughFeignToWebInterceptor() throws Exception {
        try (ConfigurableApplicationContext webContext =
                new SpringApplicationBuilder(WebEndpointConfiguration.class)
            ...
            try {
                Request request =
                        Request.create(
                                ...
                                null);
                RpcFeignClient client =
                        new RpcFeignClient(
                                new Client.Default(null, null),
                                new RpcHookChain(Collections.emptyList()),
                                new MdcRpcTracerBridge(),
                                properties);

                Response response = client.execute(request, new Request.Options());

                assertThat(response.status()).isEqualTo(200);
                assertThat(webContext.getBean(TraceEndpoint.class).traceId())
                        .isEqualTo("feign-trace-id");
                assertThat(webContext.getBean(TraceEndpoint.class).requestId())
                        .isEqualTo("feign-request-id");
            } finally {
                MDC.clear();
            }
        }

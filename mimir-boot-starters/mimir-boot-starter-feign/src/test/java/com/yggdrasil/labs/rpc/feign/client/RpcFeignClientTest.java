...
class RpcFeignClientTest {
                ...
                .execute(
                        argThat(
                                arg -> {
                                    ...
                                    return newHeaders.containsKey("trace-id")
                                            && newHeaders.containsKey("span-id");
                                }),
                        any());
        verify(hook).before(any());
        verify(hook).after(any(), any(RpcCallResult.class));
    ...
    void shouldKeepAllSafeHeaderValuesInIterationOrder() throws Exception {
        ...
        Assertions.assertEquals(
                "a,b", contextCaptor.getValue().getMetadata().getAttachments().get("Content-Type"));
    ...
    void shouldOnlyLogSanitizedUrlsAcrossEnabledDisabledAndFailureBranches() throws Exception {
        ...
        try {
            ...
            Request credentialed =
                    request(
                            "https://user:password@api.example.test:8443/orders?token=secret#detail");
            ...
            when(delegate.execute(same(opaque), any()))
                    .thenThrow(new IOException("delegate failed"));
            Assertions.assertThrows(
                    IOException.class, () -> client.execute(opaque, new Request.Options()));
    ...
    void shouldNotAttachRawFailureThrowableToDebugLogEvent() throws Exception {
        ...
        try {
            properties.setContextPropagationEnabled(false);
            Request request =
                    request("https://user:password@api.example.test:8443/orders?token=secret");
            IllegalStateException failure =
                    new IllegalStateException(
                            "request failed:"
                                + " https://user:password@api.example.test:8443/orders?token=secret");
            when(delegate.execute(same(request), any())).thenThrow(failure);
            Request.Options options = new Request.Options();

            Assertions.assertThrows(
                    IllegalStateException.class, () -> client.execute(request, options));
                    ...
                    appender.list.stream()
                            .filter(
                                    loggingEvent ->
                                            loggingEvent
                                                    .getFormattedMessage()
                                                    .contains("HTTP call failed"))
                            .findFirst()
                            .orElseThrow();
            Assertions.assertNull(event.getThrowableProxy());
            Assertions.assertTrue(
                    event.getFormattedMessage().contains("error=IllegalStateException"));
            Assertions.assertTrue(
                    event.getFormattedMessage().contains("https://api.example.test:8443/orders"));
    ...
    private Request request(String url) {
        return Request.create(
                Request.HttpMethod.GET, url, Map.of(), null, StandardCharsets.UTF_8, null);
    }
        ...
        verify(delegate)
                .execute(
                        argThat(
                                arg -> {
                                    Map<String, Collection<String>> newHeaders = arg.headers();
                                    return newHeaders.containsKey("original")
                                            && newHeaders.containsKey("trace-id");
                                }),
                        any());
        verify(hook).before(any());
        verify(hook).after(any(), any(RpcCallResult.class));
    ...
    void shouldNotCallDelegateWhenBeforeFailsAndShouldCleanUp() {
        ...
        client =
                new RpcFeignClient(
                        delegate, new RpcHookChain(List.of(failingHook)), tracerBridge, properties);
        Request request =
                Request.create(
                        Request.HttpMethod.GET,
                        "http://example.com/api",

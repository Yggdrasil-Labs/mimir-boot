 ...
 * Web 层自动配置集成测试
 *
 * <p>验证 WebAutoConfiguration 在 Servlet Web 环境下的 Bean 注册行为
 *
...
class WebAutoConfigurationIT {
    ...
    void disabledResponseEnhancementPreservesPrimaryApplicationRegistrations() {
        runner.withConfiguration(
                ...
                .withBean(
                        "applicationWebMvcRegistrations",
                        WebMvcRegistrations.class,
                        () -> new WebMvcRegistrations() {},
                        definition -> definition.setPrimary(true))
                .withBean(DisabledResponseController.class, DisabledResponseController::new)
                .run(
                        ctx -> {
                            assertThat(ctx).hasNotFailed();
                            MockMvcBuilders.webAppContextSetup(ctx.getSourceApplicationContext())
                                    .build()
                                    .perform(
    ...
    void customResponseBodyEnhancerOverridesDefault() {
        runner.withBean(
                        ResponseBodyEnhancer.class,
                        () -> new ResponseBodyEnhancer(new WebProperties()))
    ...
    void responseAdviceDoesNotIntroduceWebMvcRegistrations() {
        runner.run(
                ctx -> {
                    ...
                    assertThat(ctx)
                            .hasSingleBean(
                                    ResponseBodyEnhancerAdviceFactory.MvcResponseBodyEnhancer
                                            .class);
                });
    }
        ...
        runner.withBean(
                ...
                .run(
                        ctx -> {
                            ...
                            assertThat(ctx.getBean(WebMvcRegistrations.class))
    ...
    void requestMappingHandlerMappingFromApplicationWebMvcRegistrationsIsPreserved() {
        ...
        runner.withConfiguration(
                ...
                .run(
                        ctx -> {
                            assertThat(ctx).hasNotFailed();
                            assertThat(ctx.getBean("requestMappingHandlerMapping"))
                                    .isSameAs(applicationMapping);
                        });
    }

    @Test
    void exceptionResolverPreservesExplicitReturnValueHandlers() {
        HandlerMethodReturnValueHandler customReturnValueHandler =
        ...
        ExceptionHandlerExceptionResolver applicationResolver =
    ...
    void internalAdviceCannotBeIndependentlyComponentScanned() {
        ...
        assertThat(scanner.findCandidateComponents("com.yggdrasil.labs.web"))
                .noneMatch(
                        bean ->
                                ResponseBodyEnhancerAdviceFactory.MvcResponseBodyEnhancer.class
                                        .getName()
        ...
        runner.withConfiguration(AutoConfigurations.of(JacksonAutoConfiguration.class))
                .withBean(
                        ...
                        () ->
                                new SimpleModule()
                                        .addSerializer(
                                                ConsumerValue.class, new ConsumerValueSerializer()))
                .run(
                        ctx -> {
                            ObjectMapper objectMapper = ctx.getBean(ObjectMapper.class);

                            assertThat(objectMapper.writeValueAsString(new ConsumerValue()))
                                    .isEqualTo("\"consumer-module\"");
                            assertThat(
                                            objectMapper.writeValueAsString(
                                                    LocalDateTime.of(2026, 8, 12, 9, 30, 0)))
    ...
    @ComponentScan(
            ...
            excludeFilters =
                    @ComponentScan.Filter(
                            ...
                            pattern = ".*ResponseBodyEnhancer.*Test.*"))
    private static class WideComponentScanConfiguration {}

    private static final class ConsumerValueSerializer
            extends com.fasterxml.jackson.databind.JsonSerializer<ConsumerValue> {

        @Override
        public void serialize(
                ConsumerValue value, JsonGenerator generator, SerializerProvider serializers)
                throws IOException {

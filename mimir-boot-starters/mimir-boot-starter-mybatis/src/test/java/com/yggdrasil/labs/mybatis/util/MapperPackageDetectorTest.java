...
class MapperPackageDetectorTest extends BaseUnitTest {
    ...
    void testDetectMapperPackages_includesAllPackages() {
        ...
        for (String pkg : packages) {
            ...
            String packageWithoutWildcard =
                    pkg.replace(MybatisConstants.PACKAGE_WILDCARD_SUFFIX, "");
            assertTrue(
    ...
    void deduplicatesDefaultAndExternalPackages() throws Exception {
        Set<String> packages =
                MapperPackageDetector.detectMapperPackages(
                        resource(
                                "default mapper",
                                "jar:file:/fixtures/default.jar!/com/yggdrasil/labs/order/mapper/OrderMapper.class"),
                        resource(
                                "external mapper",
                                "jar:file:/fixtures/external.jar!/org/example/order/mapper/OrderMapper.class"),
                        resource(
                                "external duplicate",
                                "jar:file:/fixtures/external.jar!/org/example/order/mapper/OrderMapper.class"));
    ...
    void warnsAndSkipsMalformedResource() throws Exception {
        ListAppender<ILoggingEvent> appender =
                LogTestUtils.setupLogger(MapperPackageDetector.class.getName());
        try {
            Set<String> packages =
                    MapperPackageDetector.detectMapperPackages(
                            resource(
                                    "jar:file:/fixtures/bad.jar!broken/mapper/UserMapper.class",
                                    malformedUrl(
                                            "jar:file:/fixtures/bad.jar!broken/mapper/UserMapper.class")),
                            resource(
                                    "good resource",
                                    "jar:file:/fixtures/good.jar!/org/example/order/mapper/OrderMapper.class"));
        ...
        try {
            ...
            assertTrue(
                    appender.list.stream()
                            ...
                            .anyMatch(
                                    message ->
                                            message.contains("resource=null")
                                                    && message.contains("reason=")));
        } finally {
            LogTestUtils.cleanupLogger(MapperPackageDetector.class.getName(), appender);
        }
                ...
                new URLStreamHandler() {
                    ...
                    protected URLConnection openConnection(URL url) {
                        throw new UnsupportedOperationException(
                                "resource URL is only used for its external form");
                    }
                });
    }
    ...
    void testExtractPackageFromUrl_variousFormats(String url, String expectedPackage)
            throws Exception {
        Method method =
                MapperPackageDetector.class.getDeclaredMethod(
    ...
    private static Stream<Arguments> provideExtractPackageFromUrlTestCases() {
        return Stream.of(
                ...
                // 不包含 /mapper/ 的 URL
                Arguments.of("file:/path/to/target/classes/com/example/User.class", null),
                // mapperIndex <= 0 的情况
                Arguments.of("/mapper/UserMapper.class", null),
                // 空字符串
                Arguments.of("", null)
    ...
    void testExtractPathBeforeMapper(String url, int mapperIndex, String expected)
            throws Exception {
        Method method =
                MapperPackageDetector.class.getDeclaredMethod(
    ...
    private static Stream<Arguments> provideExtractPathBeforeMapperTestCases() {
        ...
        return Stream.of(
                // 普通 file URL
                Arguments.of(fileUrl, fileMapperIndex, "file:/path/to/target/classes/com/example"),
                // jar URL
                Arguments.of(jarUrl, jarMapperIndex, "com/example"),
                Arguments.of(jarUrlEdge, jarEdgeMapperIndex, ""));
    }
    ...
    /**
     * 测试第 61-64 行的分支逻辑： - extractPackageFromResource 返回 null 的情况（应该被 StringUtils.hasText 过滤） -
     * extractPackageFromResource 返回空字符串的情况（应该被 StringUtils.hasText 过滤） - extractPackageFromResource
     * 返回非空但不以 .mapper 结尾的情况（应该被 endsWith 过滤） - extractPackageFromResource 返回有效包名的情况（应该被添加到
     * detectedPackages）
     */
    @Test
    void testDetectMapperPackages_filtersInvalidPackageNames() throws Exception {
        ...
        assertFalse(StringUtils.hasText(nullResult), "StringUtils.hasText(null) 应该返回 false");

        // 测试 extractPackageFromResource 返回空字符串的情况
        Method extractUrlMethod =
                MapperPackageDetector.class.getDeclaredMethod(
                        "extractPackageFromUrl", String.class);
        extractUrlMethod.setAccessible(true);

        String emptyResult =
                (String) extractUrlMethod.invoke(null, "file:/path/to/classes/User.class");
        // 如果返回空字符串或 null，应该被 StringUtils.hasText 过滤
        if (emptyResult != null) {
            assertFalse(StringUtils.hasText(""), "StringUtils.hasText(\"\") 应该返回 false");
        }

        // 测试 extractPackageFromResource 返回非空但不以 .mapper 结尾的情况
        String nonMapperPackage = "com.example.service";
        assertTrue(StringUtils.hasText(nonMapperPackage), "非空字符串应该通过 hasText 检查");
        assertFalse(
                nonMapperPackage.endsWith(MybatisConstants.MAPPER_PACKAGE_SUFFIX),
                "不以 .mapper 结尾的包应该被 endsWith 过滤");

        // 测试 extractPackageFromResource 返回有效包名的情况
        String validMapperPackage = "com.example.mapper";
        assertTrue(StringUtils.hasText(validMapperPackage), "有效包名应该通过 hasText 检查");
        assertTrue(
                validMapperPackage.endsWith(MybatisConstants.MAPPER_PACKAGE_SUFFIX),
    ...
    void testDetectMapperPackages_filtersDefaultPackagePrefix() throws Exception {
        ...
        // 测试默认包前缀的包（应该被过滤）
        String defaultPackageUrl =
                "file:/path/to/target/classes/com/yggdrasil/labs/example/mapper/UserMapper.class";
        ...
        // 测试非默认包前缀的包（应该被包含）
        String otherPackageUrl =
                "file:/path/to/target/classes/com/other/example/mapper/UserMapper.class";
    ...
    void testExtractPathBeforeMapper_withMultipleJarSeparators() throws Exception {
        ...
        String url =
                "jar:file:/path/to/app.jar!/BOOT-INF/lib/dependency.jar!/com/example/mapper/UserMapper.class";

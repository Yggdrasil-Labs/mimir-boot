...
class ReflectionUtilsTest extends BaseUnitTest {
    ...
    /** 测试 ClassNotFoundException 异常分支 验证当类不存在时，方法能捕获异常并返回 null */
    @Test
    void testCreatePaginationInnerInterceptor_handlesClassNotFoundException() {
        ...
        // 如果类存在，应该返回实例
        assertTrue(
                result == null || result instanceof InnerInterceptor,
                "方法应该返回 null 或 InnerInterceptor 实例，不会抛出异常");
    }

    /** 测试 NoSuchMethodException 异常分支 验证当类没有无参构造方法时，方法能捕获异常并返回 null */
    @Test
    ...
    /**
     * 测试 ReflectiveOperationException 异常分支 包括
     * InstantiationException、IllegalAccessException、InvocationTargetException 等
    ...
    /** 测试 ClassCastException 异常分支 验证当类型转换失败时，方法能捕获异常并返回 null */
    @Test
    ...
    void testCreatePaginationInnerInterceptor_returnsNullOrValidInstance() {
        ...
        if (interceptor != null) {
            assertInstanceOf(InnerInterceptor.class, interceptor, "如果返回实例，应该是 InnerInterceptor 类型");
            // 验证类名正确
            assertEquals(

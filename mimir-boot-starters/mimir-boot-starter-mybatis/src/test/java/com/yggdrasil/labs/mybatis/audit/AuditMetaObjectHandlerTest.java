 ...
 * 审计元对象处理器测试
 *
 * <p>由于 MyBatis-Plus 的 strictInsertFill 和 strictUpdateFill 需要完整的 MyBatis-Plus 环境（包括
 * TableInfo），在单元测试中难以模拟，本测试主要 验证 AuditMetaObjectHandler 的异常处理和 safeAuditor 方法的逻辑。
 *
...
class AuditMetaObjectHandlerTest extends BaseUnitTest {
    ...
    private static Stream<Arguments> provideHandlerCreationScenarios() {
        return Stream.of(Arguments.of("null"), Arguments.of("exception"), Arguments.of("normal"));
    }
    ...
    void auditorProviderFailure_fallsBackToSystemAndWritesWarn() throws Exception {
        ...
        try {
            java.lang.reflect.Method method =
                    AuditMetaObjectHandler.class.getDeclaredMethod("safeAuditor");

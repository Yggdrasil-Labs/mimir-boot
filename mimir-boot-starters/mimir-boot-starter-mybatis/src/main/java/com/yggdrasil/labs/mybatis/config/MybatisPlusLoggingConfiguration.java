 ...
 *
 * <p>注意：必须在 MybatisPlusAutoConfiguration 之前加载， 这样 JsonSqlLogInnerInterceptor Bean 才能被注入到
 * MybatisPlusInterceptor 中。
 */
@AutoConfiguration
...
public class MybatisPlusLoggingConfiguration {
    ...
    private static final org.slf4j.Logger LOGGER =
            org.slf4j.LoggerFactory.getLogger(MybatisPlusLoggingConfiguration.class);
     ...
     * 注册 JSON SQL 日志拦截器
     *
     * <p>启用条件： 1. 如果显式配置了 enable-json-sql-log，则使用配置值 2. 如果未配置，则在 dev、local、test 环境自动启用 3. 可通过
     * mimir.boot.mybatis.enable-json-sql-log=false 显式禁用
     */
    @Bean
    ...
    public InnerInterceptor jsonSqlLogInnerInterceptor() {
        ...
        if (enableJson == null) {
            ...
            boolean isDevOrTest =
                    environment.acceptsProfiles(Profiles.of("dev", "local", "development", "test"));
            if (!isDevOrTest) {

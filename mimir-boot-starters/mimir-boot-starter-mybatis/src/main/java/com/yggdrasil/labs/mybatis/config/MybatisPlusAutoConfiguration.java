 ...
 *
 * <p>分页拦截器：从 MyBatis-Plus 3.5.9 开始，分页插件需要单独引入 mybatis-plus-jsqlparser 依赖。 本 Starter
 * 已自动包含该依赖，使用 @ConditionalOnClass 确保类存在时才创建 Bean。
 *
 * <p>注意：此自动配置类需要在 MyBatis-Plus 的自动配置之前加载，以确保 MapperScannerConfigurer 能够被正确创建。如果 MyBatis-Plus
 * 的自动配置先创建了 MapperScannerConfigurer， 此配置类中的 mapperScannerConfigurer
 * 方法将不会执行（因为 @ConditionalOnMissingBean）。
 */
@AutoConfiguration
...
public class MybatisPlusAutoConfiguration {
    ...
    public MybatisPlusInterceptor mybatisPlusInterceptor(
        ...
        try {
            String[] beanNames =
                    beanFactory.getBeanNamesForType(InnerInterceptor.class, false, false);
    ...
    private static String normalizeMapperScanPackage(String packagePattern) {
        if (packagePattern.endsWith(MybatisConstants.PACKAGE_WILDCARD_SUFFIX)) {
            return packagePattern.substring(
                    0, packagePattern.length() - MybatisConstants.PACKAGE_WILDCARD_SUFFIX.length());
        }
     ...
     * 配置 MyBatis 日志实现。
     *
     * <p>强制使用 SLF4J 作为 MyBatis 的日志实现，确保日志统一管理。
     *
     * <p>是否打印 SQL 由日志级别控制：
     *
     * <ul>
     *   <li>在 logback-spring.xml 中配置 Mapper 接口的日志级别
     *   <li>例如：{@code <logger name="com.example.mapper" level="DEBUG"/>} 会打印 SQL
     *   <li>例如：{@code <logger name="com.example.mapper" level="INFO"/>} 不会打印 SQL
     * </ul>
     *
     * <p>这样确保在其他项目引入此 starter 时，MyBatis 永远使用 slf4j， 并且可以通过日志框架统一控制是否打印 SQL。
     *

...
public enum SensitiveDataPattern {
    ...
    PASSWORD(
            "password",
            "(?i)((?:password|pwd|passwd|%70assword|密码)\\\"?\\s*[=:])\\s*(?:\\\"[^\\\"]*\\\"|'[^']*'|[^,}\\]\\s]+)"),

    /** Token 相关字段 */
    TOKEN(
            "token",
            "(?i)((?:token|access_token|refresh_token)\\\"?\\s*[=:])\\s*(?:\\\"[^\\\"]*\\\"|'[^']*'|[^,}\\]\\s]+)"),

    /** 密钥相关字段 */
    SECRET(
            "secret",
            "(?i)((?:secret|private_key|privateKey|secret_key|secretKey|access_key|accessKey|%73ecretKey|私钥)\\\"?\\s*[=:])\\s*(?:\\\"[^\\\"]*\\\"|'[^']*'|[^,}\\]\\s]+)"),
    ...
    private static final List<String> TOKEN_KEYS =

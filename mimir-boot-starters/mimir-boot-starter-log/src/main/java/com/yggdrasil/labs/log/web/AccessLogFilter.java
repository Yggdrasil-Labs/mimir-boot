 ...
 *
 * <ul>
 *   <li>记录每个请求的详细信息：IP、URI、耗时、状态码
 *   <li>根据耗时判断是否为慢接口，慢接口输出 WARN 级别日志
 *   <li>慢接口阈值可配置
 *   <li>记录容器提供的直连对端 IP
 * </ul>
 *
...
public class AccessLogFilter implements Filter {
    ...
    private static final String LIFECYCLE_ATTRIBUTE =
            AccessLogFilter.class.getName() + ".lifecycle";
        ...
        try {
            ...
            if (!async) {
                lifecycle.terminate(
                        httpResponse,
                        -1,
                        failure == null ? "COMPLETED" : "ERROR",
                        failure == null ? "-" : failure.getClass().getName());
            }
        }
    }

    private RequestLifecycle lifecycleFor(
    ...
    private final class RequestLifecycle {
        ...
        private final AtomicReference<LifecycleState> state =
                new AtomicReference<>(LifecycleState.initial());
        ...
        private void safeTerminate(
                AsyncEvent event, int registrationGeneration, String outcome, String errorType) {
        ...
        private void safeTerminate(
                HttpServletResponse terminalResponse,
            ...
            try {
                ...
                logAccessByStatus(
                        new AccessLogEntry(
                                ...
                                new TerminalState(outcome, errorType)));
            } catch (Exception e) {
        ...
        private void logAccessByStatus(AccessLogEntry entry) {
            ...
            // 使用参数化日志，防止日志注入攻击
            String message =
                    "IP=[{}], Method=[{}], URI=[{}], Status=[{}], Outcome=[{}], ErrorType=[{}], "
            ...
            if (entry.statusCode() >= 500) {
                ...
                // 4xx: 客户端错误，记录为 WARN
                // 示例：400 Bad Request, 401 Unauthorized, 403 Forbidden, 404 Not Found, 429 Too Many
                // Requests
                if (isSlow) {
    ...
    private record LifecycleState(Phase phase, int generation, List<AsyncContext> claimedContexts) {
        ...
        private LifecycleState claim(AsyncContext context) {
            List<AsyncContext> claimed =
                    contains(context) ? claimedContexts : new ArrayList<>(claimedContexts);
            if (!contains(context)) {
                claimed.add(context);
            }
    ...
    /**
     * 判断是否应该记录访问日志 如果请求路径匹配排除列表中的任何模式，则不记录
     *
     ...
     * 获取容器提供的直连对端 IP。
     *
     * <p>转发头仅应由已建立可信边界的容器改写为 remoteAddr 后使用。
     *
     * @param request HTTP 请求
     * @return 客户端真实 IP
     */
     ...
     * 清理用户输入，防止日志注入攻击
     *
     * <p>移除换行符、回车符、制表符等控制字符，防止恶意用户通过构造特殊字符来伪造日志条目
     *
     * @param input 原始输入

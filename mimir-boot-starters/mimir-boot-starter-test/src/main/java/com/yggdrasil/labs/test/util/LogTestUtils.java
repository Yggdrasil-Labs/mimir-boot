 ...
 * 日志测试工具类
 *
 * <p>提供日志测试的常用工具方法，简化 Logback 日志测试的设置和验证。
 *
 * <p>使用示例：
 *
 * <pre>{@code
...
public final class LogTestUtils {
    ...
    public static ListAppender<ILoggingEvent> setupLogger(String loggerName) {
        ...
        LOGGER_STATES.put(
                appender, new LoggerState(logger, logger.getLevel(), logger.isAdditive()));
     ...
     */
    private static ILoggingEvent validateAndGetEvent(
            ListAppender<ILoggingEvent> appender, int index) {
     ...
     */
    public static void assertLogLevel(
            ListAppender<ILoggingEvent> appender, int index, Level expectedLevel) {
     ...
     */
    public static void assertLogContains(
            ListAppender<ILoggingEvent> appender, int index, String expectedText) {
     ...
     */
    public static void assertLogStatus(
            ListAppender<ILoggingEvent> appender, int index, int expectedStatus) {
     ...
     * 获取已捕获的日志数量。
     *
     * <p>ListAppender 的事件列表访问集中在此 Logback 适配边界，通用断言工具不依赖其内部字段。
     *
     * @param appender ListAppender 实例
     * @return 已捕获的日志数量
     */

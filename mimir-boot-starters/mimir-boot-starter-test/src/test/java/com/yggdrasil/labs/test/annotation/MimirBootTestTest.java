...
class MimirBootTestTest {
    ...
    void testMimirBootTestAnnotation_Present() {
        // 验证注解存在
        assertTrue(
                TestClassWithAnnotation.class.isAnnotationPresent(MimirBootTest.class),
    ...
    void testMimirBootTestAnnotation_MetaAnnotations() {
        ...
        assertTrue(
                AnnotatedElementUtils.isAnnotated(
                        TestClassWithAnnotation.class, SpringBootTest.class),
                "@MimirBootTest 应包含 @SpringBootTest");
        assertTrue(
                AnnotatedElementUtils.isAnnotated(
                        TestClassWithAnnotation.class, ActiveProfiles.class),
                "@MimirBootTest 应包含 @ActiveProfiles");

        // 验证 ActiveProfiles 的值（使用 AnnotationUtils 来获取组合注解中的元注解）
        ActiveProfiles activeProfiles =
                AnnotationUtils.findAnnotation(TestClassWithAnnotation.class, ActiveProfiles.class);
        assertNotNull(activeProfiles, "ActiveProfiles 注解不应为 null");
        assertArrayEquals(
                new String[] {"test"}, activeProfiles.value(), "ActiveProfiles 值应为 ['test']");
    }
    ...
    void testMimirBootTestAnnotation_CustomValues() {
        MimirBootTest annotation =
                TestClassWithCustomProperties.class.getAnnotation(MimirBootTest.class);
        assertNotNull(annotation, "注解不应为 null");
        ...
        assertArrayEquals(
                ...
                annotation.excludeAutoConfiguration(),
                "保留的兼容属性应继续接受旧调用方赋值");
        assertEquals(
    ...
    void testMimirBootTestAnnotation_KeepsDeprecatedCompatibilityAttributes() {
        var useDefaultFilters =
                java.util.Arrays.stream(MimirBootTest.class.getDeclaredMethods())
                        .filter(method -> method.getName().equals("useDefaultFilters"))
                        .findFirst();
        var excludeAutoConfiguration =
                java.util.Arrays.stream(MimirBootTest.class.getDeclaredMethods())
        ...
        assertTrue(excludeAutoConfiguration.isPresent(), "应保留 excludeAutoConfiguration 以兼容既有测试源码");
        assertArrayEquals(
                new Class<?>[0],
    ...
    private void assertAliasForSpringBootTest(String methodName, String targetAttribute)
            throws NoSuchMethodException {
        AliasFor aliasFor =
                MimirBootTest.class.getDeclaredMethod(methodName).getAnnotation(AliasFor.class);
